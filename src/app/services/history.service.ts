...
import { ActionType, HistoryAction, HistoryState } from '../models/history.model';
...
export class HistoryService {
  ...
  addAction(action: Omit<HistoryAction, 'timestamp'>): void {
    ...
    this.historyState.update(state => {
      ...
      const actions = state.currentIndex < state.actions.length - 1
        ? state.actions.slice(0, state.currentIndex + 1)
        : state.actions;
