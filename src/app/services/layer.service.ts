...
export class LayerService {
  ...
  selectLayer(id: string, addToSelection: boolean = false): void {
    ...
    if (addToSelection) {
      this.selectedLayerIds.update(ids => 
  ...
  ungroupLayers(groupId: string): void {
    ...
    this.layers.update(layers => [
      ...layers.filter(layer => layer.id !== groupId),
      ...childLayers
    ]);
  ...
  private renderLayer(ctx: CanvasRenderingContext2D, layer: Layer): void {
    ...
    switch (layer.type) {
      ...
      case LayerType.DIAGRAM:
        this.renderDiagramLayer(ctx, layer);
        break;
    }
  ...
  /**
   * Render a diagram layer
   */
  private renderDiagramLayer(ctx: CanvasRenderingContext2D, layer: Layer): void {
    if (layer.type !== LayerType.DIAGRAM || !layer.data) return;
    
    // Rendering diagrams is complex and would depend on the specific diagram type
    // This is a simplified placeholder
    
    const { type, elements, connections } = layer.data;
    
    // Render diagram elements
    elements.forEach((element:any) => {
      // Render element based on type (class, actor, entity, etc.)
      // ...
    });
    
    // Render connections between elements
    connections.forEach((connection:any) => {
      // Render connection lines, arrows, labels, etc.
      // ...
    });
  }
